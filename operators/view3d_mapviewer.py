...
class VIEW3D_OT_map_start(Operator):
	...
	def ensureValidEnums(self):
		"""Ensure interdependent enums (src/lay/grd) hold valid values (fix empty enum value bug on newer Blender)"""
		if not self.src or self.src not in SOURCES:
			self.src = next(iter(SOURCES))
		srcDef = SOURCES[self.src]
		if not self.lay or self.lay not in srcDef['layers']:
			self.lay = next(iter(srcDef['layers']))
		if not self.grd or self.grd not in GRIDS:
			self.grd = srcDef['grid']
