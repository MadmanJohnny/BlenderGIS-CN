...
DEFAULT_OVERPASS_SERVER =  [
	("https://overpass.kumi.systems/api/interpreter", 'overpass.kumi.systems', 'Reliable Overpass API mirror'),
	("https://z.overpass-api.de/api/interpreter", 'overpass-api.de (z)', 'Overpass API mirror'),
	("https://lz4.overpass-api.de/api/interpreter", 'overpass-api.de (lz4)', 'Overpass API mirror for large queries')
]
...
class BGIS_PREFS(AddonPreferences):
	...
	def updateTiandituApiKey(self, context):
		settings.tianditu_api_key = self.tianditu_api_key

	tianditu_api_key: StringProperty(
		name = "",
		description = "API key for Tianditu (天地图) map service",
		update = updateTiandituApiKey
	)
	...
	def draw(self, context):
		...
		row.prop(self, "tianditu_api_key")
