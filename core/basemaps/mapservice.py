...
from ..proj.gcj02 import shiftBboxToGcj02, shiftBboxToWgs84
...
class MapService():
	...
	def __init__(self, srckey, cacheFolder, dstGridKey=None):
		...
		for k, v in source.items():
			setattr(self, k, v)
		self.gcj02 = source.get('gcj02', False)
	...
	def buildUrl(self, laykey, col, row, zoom):
		...
		if self.service == 'TMS':
			...
			url = url.replace("{STYLE}", getattr(lay, 'style', ''))
		...
		#API key placeholder (used by Tianditu and other keyed services)
		url = url.replace("{TK}", settings.tianditu_api_key or "")
	...
	def getImage(self, laykey, bbox, zoom, path=None, bigTiff=False, outCRS=None, toDstGrid=True, nbThread=10, cpt=True):
		...
		#GCJ-02 correction : shift bbox from WGS84 to GCJ-02 Web Mercator for tile request
		gcj02 = self.gcj02 and not toDstGrid and tm.CRS == 'EPSG:3857'
		if gcj02:
			bbox = shiftBboxToGcj02(bbox)
		...
		#GCJ-02 correction : restore mosaic georeference from GCJ-02 to WGS84 Web Mercator
		if gcj02 and not bigTiff:
			gr = mosaic.georef
			w, h = gr.rSize.x, gr.rSize.y
			gb = gr.bbox
			wxmin, wymin, wxmax, wymax = shiftBboxToWgs84((gb.xmin, gb.ymin, gb.xmax, gb.ymax))
			resx = (wxmax - wxmin) / w
			resy = (wymin - wymax) / h
			mosaic.georef = GeoRef((w, h), (resx, resy), (wxmin, wymax), pxCenter=False, crs=gr.crs)
